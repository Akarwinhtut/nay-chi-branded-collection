...
export function ProductTile({
  ...
  return (
    <Link href={`/services/${product.slug}`} className={`group block ${className}`}>
      <article className="space-y-6 transition-transform duration-300 group-hover:-translate-y-0.5">
        ...
        <div className="space-y-5 px-1">
          ...
          <div className="grid gap-3 border-t border-[var(--color-line)] pt-4 sm:grid-cols-[1fr_auto] sm:items-end">
            <h3 className="font-display text-[2.75rem] font-medium leading-[0.88] tracking-[-0.04em] text-[var(--color-ink)] sm:text-[3.2rem]">
              {product.name}
            </h3>
